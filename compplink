...
import os
plinkpath = input("PLINK path:")
plinkpath = plinkpath.strip()
if not os.path.isfile(plinkpath):
    raise Exception("Hey, '{}' does not exist!".format(plinkpath))
elif not os.access(plinkpath, os.X_OK):
    raise Exception("Hey, '{}' is not executable!".format(plinkpath))
...
completion_path = "/etc/bash_completion.d/%s" % plinkname
if os.path.isfile(completion_path):
    print("Completion script %s already exists." % completion_path)
    comp_reply = input("Type yes if you want to overwrite: ")
    if comp_reply.strip() != "yes":
        print("I leave the original file alone, as you have requested. Bye!")
        exit(0)
        
try:
    with open(completion_path, "w") as fh:
        fh.write(completion_string)
